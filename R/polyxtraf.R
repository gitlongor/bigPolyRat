...
predict.polynomialf <-
  ...
  {
    if(!is.polynomialf(newdata)) newdata=as.bigf(newdata)
    p <- object
...
GCD.polyqlist <- GCD.polynomialf


#LCM <- function(...)   UseMethod("LCM")

LCM.polynomialf <-LCM.polynomialz
function(...) {
  args <- c.polyqlist(...)
  if(length(args) < 2)
    stop("Need at least two polynomials.")
...
LCM.polyqlist <- LCM.polynomialf

#decartes = function(p) UseMethod('decartes')
decartes.mpfr = decartes.bigz
decartes.polynomialf = decartes.polynomialz
