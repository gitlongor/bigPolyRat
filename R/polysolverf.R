...
uBound.polynomialf=uBound.polynomialq #=function(p, method=c('Cauchy', 'Lagrange', 'Kojima','Fujiwara','SumAdjRatio','Deutsch'))
# {
# e1=trimZeros(coef(p))
...
lBound.polynomialf=lBound.polynomialq#=function(p, method=c('Rouche'))
# {
# e1=trimZeros(coef(p))
# n=length(e1)
...
squareFree.polynomialf = function(p, ...)  {
  p=as.polynomialq(p)
  .Class = c(.Class, 'polynomialq')
  NextMethod(.Generic, p)
}
...
nRealRoots.polynomialf = function(p, lower=-upper, upper=uBound(p)*(1+mpfr(1L,1e22)), method='Sturm', ...){
  p=as.polynomialq(p)
  .Class = c(.Class, 'polynomialq')
  NextMethod(.Generic, p)
}
...
realRootIso.polynomialz=function(p, lower=-upper, upper=uBound(p)*(1+mpfr(1L,1e22)), eps=mpfr(1, 1e3), method='bisect',...){
  p=as.polynomialq(p)
  .Class = c(.Class, 'polynomialq')
  NextMethod(.Generic, p)
}