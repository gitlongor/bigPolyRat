...
ratlist.rationalf=function(...)
{
  lst=list(...)
  cls=sapply(lst, class)
  if(!all(cls=='rationalf')) {
    if(all(grepl('^rational[qz]$', cls))) {
      do.call('ratlist', lapply(lst, as.rationalf))
    }else stop('unknown object classes')
  }else structure(lst, class='ratlist')
}
...
ratlist.polyflist=function(
  numerList, 
  denomList=rep(polynomialf(1), length.out=length(numerList)), 
  ...) NULL
body(ratlist.polyflist) = body(ratlist.polyzlist)
